...
TTestIS <- setRefClass(
    ...
    methods=list(
        run=function() {
            ...
            equality <- .results$get("equalityofv")
            
            groupingVariable <- .options$values()$groupingVar
            dependentVariables <- .options$values()$vars
            
            wantsStudents <- .options$values()$student
            wantsWelchs <-.options$values()$welch
            ...
            if (length(groupingVariable) > 1)
                reject("There must only be one grouping variable", code="too_many_grouping_vars")
            
            else if (any(dependentVariables == groupingVariable))
                reject("Grouping variable '{a}' must not also be a dependent variable", code="a_is_dependent_variable", a=groupingVariable)
            
            else if (length(unique(dataset[[groupingVariable]])) != 2 || length(levels(dataset[[groupingVariable]])) != 2)
                reject("Grouping variable '{a}' must have exactly 2 levels", code="grouping_var_must_have_2_levels", a=groupingVariable)
            ...
            if (naHandling == "listwise") {
                ...
                if (length(unique(dataset.clean[[groupingVariable]])) != 2)
                    reject("Grouping variable '{a}' has less than 2 levels after missing values are excluded", code="grouping_var_must_have_2_levels", a=groupingVariable)
            ...
            for (i in seq_along(dependentVariables)) {
                ...
                if (naHandling == "perAnalysis") {
                    ...
                    ## Check if removing NA has excluded all of one group in the grouping variable
                    if (length(unique(dataset.clean[[groupingVariable]])) != 2)
                        reject("Grouping variable '{a}' has less than 2 levels after missing values of dependent variable '{b}' are excluded", code="grouping_var_must_have_2_levels", a=groupingVariable, b=name)
